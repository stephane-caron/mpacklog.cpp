...
int main() {
  ...
  const std::string full_log_file = "/tmp/full_logger.mpack";
  const std::string delta_log_file = "/tmp/delta_logger.mpack";

  std::filesystem::remove(full_log_file);
  std::filesystem::remove(delta_log_file);

  mpacklog::Logger full_logger(full_log_file);
  mpacklog::DeltaLogger delta_logger(delta_log_file);
  ...
  std::uintmax_t full_file_size = std::filesystem::file_size(full_log_file);
  std::uintmax_t delta_file_size = std::filesystem::file_size(delta_log_file);

  std::cout << "\n=== Memory performance ===\n";
  std::cout << "Full logger file size: " << full_file_size << " bytes\n";
  std::cout << "Delta logger file size: " << delta_file_size << " bytes\n";
  std::cout << "Compression ratio: "
            << (static_cast<double>(full_file_size) / delta_file_size) << "\n";
  ...
  std::cout << "\n=== Time performance  ===\n";
  std::cout << "Full logger avg time per step: "
            << (full_time_total.count() / kNumSteps) << " μs\n";
  std::cout << "Delta logger avg time per step: "
